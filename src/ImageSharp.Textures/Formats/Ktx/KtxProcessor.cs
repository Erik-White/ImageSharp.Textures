...
internal class KtxProcessor
{
    ...
    {
        uint numberOfMipMaps = this.KtxHeader.NumberOfMipmapLevels != 0 ? this.KtxHeader.NumberOfMipmapLevels : 1;

        CubemapTexture cubeMapTexture = new();
        TBlock blockFormat = default;

        for (int i = 0; i < numberOfMipMaps; i++)
        {
            uint dataForEachFace = this.ReadTextureDataSize(stream);
    ...
    private MipMap<TBlock> ReadFaceTexture<TBlock>(Stream stream, int width, int height, TBlock blockFormat, uint dataForEachFace)
        where TBlock : struct, IBlock<TBlock>
    {
        ...
        where TBlock : struct, IBlock<TBlock>
    {
        ...
        TBlock blockFormat = default;
        MipMap<TBlock>[] mipMaps = new MipMap<TBlock>[count];

        for (int i = 0; i < count; i++)
        {
            uint pixelDataSize = this.ReadTextureDataSize(stream);
